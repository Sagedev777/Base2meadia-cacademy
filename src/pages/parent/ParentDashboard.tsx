...
import { STUDENTS, GRADES, TERMS, computeGpaSummary, computeAttendanceSummary } from '../../data/mockData';
import { RadarChart, Radar, PolarGrid, PolarAngleAxis, ResponsiveContainer, Tooltip } from 'recharts';

// Parent is linked to student st1 (Kwame Asante)
...
export default function ParentDashboard() {
  ...
  const gpa = childGrades.length
    ...
    : 0;
  const avgScore = childGrades.length ? Math.round(childGrades.reduce((s, g) => s + g.score, 0) / childGrades.length) : 0;
  const att = computeAttendanceSummary(CHILD.id);
  const leaderboard = computeGpaSummary(CHILD.classId, currentTerm.id);
  const childRank = leaderboard.find(l => l.studentId === CHILD.id);
  const gpaColor = gpa >= 3.5 ? '#22c55e' : gpa >= 2.5 ? '#f59e0b' : '#ef4444';

  const radarData = childGrades.map(g => ({ subject: g.subjectName.split(' ')[0], score: g.score, fullMark: 100 }));
                  ...
                  <td>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <div className="progress-bar-wrap" style={{ width: 70 }}>
                        <div className="progress-bar-fill" style={{ width: `${g.score}%`, background: g.score >= 80 ? '#22c55e' : g.score >= 60 ? '#f59e0b' : '#ef4444' }}/>
                      </div>
                      <span style={{ fontWeight: 700 }}>{g.score}%</span>
                    </div>
                  </td>
                  <td style={{ fontWeight: 800, fontSize: 18, color: g.score >= 80 ? '#22c55e' : g.score >= 60 ? '#f59e0b' : '#ef4444' }}>{g.letterGrade}</td>
