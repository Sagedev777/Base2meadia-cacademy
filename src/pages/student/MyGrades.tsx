import { GRADES, STUDENTS, TERMS, SUBJECTS } from '../../data/mockData';
...
export default function MyGrades() {
  ...
  const gpa = myGrades.length
    ...
    : 0;
  const avgScore = myGrades.length ? Math.round(myGrades.reduce((s, g) => s + g.score, 0) / myGrades.length) : 0;
  const highest = myGrades.length ? Math.max(...myGrades.map(g => g.score)) : 0;
  const lowest  = myGrades.length ? Math.min(...myGrades.map(g => g.score)) : 0;
    ...
    <div>
      ...
      <div className="stat-grid" style={{ marginBottom: 24 }}>
        {[
          ...
          { label: 'Highest Score', value: `${highest}%`, color: '#a855f7', sub: myGrades.find(g => g.score === highest)?.subjectName ?? '' },
          { label: 'Lowest Score', value: `${lowest}%`, color: '#f97316', sub: myGrades.find(g => g.score === lowest)?.subjectName ?? '' },
        ].map((s, i) => (
      ...
      <div className="grid-2" style={{ marginBottom: 24 }}>
        {myGrades.map(g => {
          const color = gradeBadgeColor(g.score);
          return (
            <div className="card" key={g.id} style={{ borderLeft: `4px solid ${color}` }}>
              ...
              <div style={{ marginTop: 14 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6, fontSize: 12 }}>
                  ...
                  <span style={{ fontWeight: 700, color }}>{g.score} / 100</span>
                </div>
                <div className="progress-bar-wrap">
                  <div className="progress-bar-fill" style={{ width: `${g.score}%`, background: color }}/>
                </div>
