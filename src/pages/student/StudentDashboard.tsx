...
import { GRADES, ATTENDANCE, STUDENTS, TERMS, computeGpaSummary, computeAttendanceSummary } from '../../data/mockData';
...
export default function StudentDashboard() {
  ...
  const leaderboard = computeGpaSummary(student.classId, currentTerm.id);
  const myRank = leaderboard.find(l => l.studentId === student.id);

  const trendData = myGrades.map(g => ({ subject: g.subjectName.split(' ')[0], score: g.score }));
