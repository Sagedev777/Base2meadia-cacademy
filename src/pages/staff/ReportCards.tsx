...
import { STUDENTS, CLASSES, TERMS, GRADES, STAFF_LIST, calcGrade, computeGpaSummary, computeAttendanceSummary } from '../../data/mockData';
...
function getStudentGpa(studentId: string, classId: string, termId: string) {
  const summaries = computeGpaSummary(classId, termId);
...
export default function ReportCards() {
  ...
  const buildCardHTML = (student: typeof STUDENTS[number]) => {
    const grades    = GRADES.filter(g => g.studentId === student.id && g.termId === currentTerm.id);
    const gpaEntry  = getStudentGpa(student.id, student.classId, currentTerm.id);
    const att       = computeAttendanceSummary(student.id);
    const cls       = CLASSES.find(c => c.id === student.classId);
    const total     = computeGpaSummary(student.classId, currentTerm.id).length;

    const gradeRows = grades.map(g => {
      const info = calcGrade(g.score);
      const col  = g.score >= 80 ? '#22c55e' : g.score >= 70 ? '#3b82f6' : g.score >= 60 ? '#f59e0b' : '#ef4444';
      return `<tr>
        <td>${g.subjectName}</td>
        <td style="font-weight:700">${g.score}%</td>
    ...
    return `
      ...
      <div class="sub">Student Academic Report Card · ${currentTerm.name}</div>
      <hr/>
      <div class="info-grid">
      ...
      <table>
        ...
        <tbody>${gradeRows || '<tr><td colspan="5" style="text-align:center;color:#999">No grades recorded</td></tr>'}</tbody>
      </table>
      <div class="sum">
                  ...
                  <table>
                    ...
                    <tbody>
                      {grades.length > 0 ? grades.map(g => {
                        const info  = calcGrade(g.score);
                        const color = g.score >= 80 ? '#22c55e' : g.score >= 70 ? '#3b82f6' : g.score >= 60 ? '#f59e0b' : '#ef4444';
                        return <tr key={g.id}>
