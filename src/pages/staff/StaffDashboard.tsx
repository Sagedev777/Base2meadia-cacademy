...
import { STAFF_LIST, GRADES, ATTENDANCE, CLASSES, TERMS } from '../../data/mockData';
...
export default function StaffDashboard() {
  ...
  const subjectAvgs = staffProfile.subjects.map(code => {
    ...
    const avg = sg.length ? Math.round(sg.reduce((s,g) => s + g.score, 0) / sg.length) : 0;
    ...
    <div>
      ...
      <div className="stat-grid">
        {[
          ...
          { icon: <Star size={22}/>,        label: 'Avg Class Score',   value: myGrades.length ? `${Math.round(myGrades.reduce((s,g)=>s+g.score,0)/myGrades.length)}%` : '—', color: '#f97316' },
        ].map((s, i) => (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {myGrades.slice(0, 6).map(g => (
              <div key={g.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 14px', background: 'var(--bg-surface)', borderRadius: 8 }}>
                ...
                <div style={{ textAlign: 'right' }}>
                  <div style={{ fontWeight: 800, fontSize: 16, color: g.score >= 80 ? '#22c55e' : g.score >= 60 ? '#f59e0b' : '#ef4444' }}>{g.score}%</div>
