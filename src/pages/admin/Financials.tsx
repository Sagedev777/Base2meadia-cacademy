...
function printInvoice(p: Payment) {
  ...
  win.document.write(`
    ...
    <body>
      ...
      <hr class="divider"/>
      <div class="row"><span class="lbl">Total Fee</span><span class="val">GH₵ ${p.totalFee.toLocaleString()}</span></div>
      <div class="row"><span class="lbl">Amount Paid</span><span class="val" style="color:#16a34a">GH₵ ${p.amountPaid.toLocaleString()}</span></div>
      <div class="row"><span class="lbl">Balance Remaining</span><span class="balance-val">GH₵ ${p.balance.toLocaleString()}</span></div>
      <div class="total-row">
  ...
  return (
    <div>
      ...
      <div className="stat-grid">
        {[
          { icon: <DollarSign size={22}/>, label: 'Total Fees Expected', value: `GH₵ ${totalFees.toLocaleString()}`, color: '#94a3b8' },
          { icon: <CheckCircle size={22}/>, label: 'Total Collected', value: `GH₵ ${totalRevenue.toLocaleString()}`, color: '#22c55e' },
          { icon: <AlertCircle size={22}/>, label: 'Outstanding Balance', value: `GH₵ ${totalOutstanding.toLocaleString()}`, color: '#ef4444' },
      ...
      <div className="card" style={{ marginBottom: 24 }}>
        <div className="card-header">
          <div><h3>Monthly Collection Overview</h3><p>Collected vs Outstanding (GH₵)</p></div>
            ...
            <tbody>
              {filtered.map(p => (
                <tr key={p.id}>
                  ...
                  <td style={{ fontWeight: 600 }}>GH₵ {p.totalFee.toLocaleString()}</td>
                  <td style={{ color: '#22c55e', fontWeight: 600 }}>GH₵ {p.amountPaid.toLocaleString()}</td>
                  <td style={{ color: p.balance > 0 ? '#ef4444' : '#22c55e', fontWeight: 600 }}>GH₵ {p.balance.toLocaleString()}</td>
