...
type FormData = Omit<Student, 'id' | 'fullName'>;

const emptyForm: FormData = {
  studentId: '', firstName: '', lastName: '', dateOfBirth: '', gender: 'Male',
  classId: 'c1', className: 'Grade 10A', enrollmentDate: new Date().toISOString().slice(0, 10),
  status: 'active', email: '', phone: '', address: '',
};
...
export default function Students() {
  ...
  const openEdit = (s: Student) => {
    setEditing(s);
    setForm({ studentId: s.studentId, firstName: s.firstName, lastName: s.lastName, dateOfBirth: s.dateOfBirth, gender: s.gender, classId: s.classId, className: s.className, enrollmentDate: s.enrollmentDate, status: s.status, email: s.email, phone: s.phone, address: s.address });
    setShowModal(true);
  };
  ...
  const handleSave = () => {
    ...
    if (editing) {
      ...
      const newSId = `B2MA-2025-${String(list.length + 1).padStart(3, '0')}`;
      setList(l => [...l, { id: newId, ...form, studentId: form.studentId || newSId, fullName, className }]);
    }
    setShowModal(false);
  };
