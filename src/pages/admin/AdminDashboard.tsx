    ...
    <div>
      ...
      <div className="stat-grid">
        {[
          ...
          { icon: <DollarSign size={22}/>, label: 'Total Revenue', value: `GH₵ ${totalRevenue.toLocaleString()}`, color: '#f97316', delta: `GH₵ ${outstanding.toLocaleString()} outstanding`, path: '/admin/financials' },
        ].map((s, i) => (
      ...
      <div style={{ marginTop: 20, background: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.2)', borderRadius: 12, padding: '14px 18px', display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <div>
          ...
          <span style={{ fontSize: 13, color: '#94a3b8' }}>
            {PAYMENTS.filter(p => p.status !== 'paid').length} students have pending payments totalling <strong style={{ color: '#fbbf24' }}>GH₵ {outstanding.toLocaleString()}</strong>
          </span>
        </div>
